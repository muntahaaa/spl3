...
def _fetch_to_local(url_or_path: str, suffix: str = "") -> Optional[Path]:
    ...
    - http(s) URL : download to a stable temp file and return that.
...
def state2json(state: dict, save_path: str = None) -> str:
    ...
    filtered = {
        ...
        "final_page": {
            ...
            "page_json_local": (
                state.get("current_page_json")
                if isinstance(state.get("current_page_json"), str) else ""
            ),
...
def record_action_to_state(
    ...
    step: int,
    screenshot_path: str,
    elements_json_url: Optional[str],
    elements_json_local: Optional[str],
...
def _load_elements_for_step(step: dict) -> tuple:
    ...
    print(f"[_load_elements_for_step] Step {step.get('step')}: no elements JSON")
    return None, []
    ...
    try:
        ...
        for step in data["history_steps"]:
            ...
            if interacted is not None:
                ...
                # swipe / back / wait have no target element by design — silence is correct.
                # Only warn for action types that are expected to have an interacted element.
                if action_type not in ("swipe", "swipe_precise", "back", "wait", "", None):
            ...
            for key in ("page_json_local", "page_json"):
                ...
                if p is not None:
                    try:
                        ...
                        print(f"[json2db] final_page load failed ({key}): {exc}")
